...
async def search_film_omdb(session: aiohttp.ClientSession, film_name: str) -> tuple[list[dict], str]:
    """
        ...
        async with session.get(base_url) as response:
            if response.status == 200:
                ...
                if data.get("Response") == "True":
                    ...
                    return [result], "Open movie database (OMDb), https://www.omdbapi.com/"
                else:
...
async def search_film_kinopoisk(session: aiohttp.ClientSession, film_name: str) -> tuple[list[dict], str]:
    """
    Seach film information using OMDB API.
    """
    ...
    try:
        ...
        for film in films_data:
            ...
            if isinstance(film_result['genre'], list):
                film_result['genre'] = ', '.join([genre['genre'] for genre in film_result['genre']])
